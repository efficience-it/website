...
Vue.js adopte une approche progressivement adoptable. On peut l'intégrer dans une page existante via un simple tag script, puis monter en puissance vers une SPA complète avec routeur, store et rendu serveur via Nuxt. Pour aller plus loin sur les applications web modernes, notre article sur l'[amélioration de l'expérience utilisateur avec le manifeste des applications web](/article/ameliorer-lexperience-utilisateur-grace-au-manifeste-des-applications-web) couvre la transformation de ces SPA en PWA installables. Cette progressivité n'est pas un compromis : c'est une décision architecturale qui réduit le coût d'entrée sans sacrifier les capacités avancées.

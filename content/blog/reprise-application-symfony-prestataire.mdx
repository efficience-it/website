---
title: "Reprendre une application Symfony mal codée par un autre prestataire : la méthode"
date: "2026-06-26"
updatedAt: "2026-06-26"
author: "Louis-Arnaud Catoire"
category: "Architecture"
excerpt: "Vous avez hérité d'une application Symfony bancale livrée par un ancien prestataire ? La méthode en quatre phases pour reprendre le contrôle, stabiliser et faire évoluer sans tout réécrire."
...
Si l'ancien prestataire ne coopère pas, rappelez-vous que le code développé sur commande vous appartient dès lors que votre contrat prévoit la cession des droits, ce qui est la norme. Une demande écrite formelle, suivie si besoin d'une mise en demeure, débloque la majorité des situations. En parallèle, un repreneur compétent peut reconstituer une partie des accès à partir de ce dont vous disposez déjà.
...
On part de l'analyse statique. [PHPStan](/article/comment-phpstan-peut-vous-aider-a-ameliorer-la-qualite-de-votre-code-php) configuré au niveau le plus élevé que la base supporte révèle en quelques minutes l'ampleur des problèmes de typage et de cohérence. On mesure ensuite la couverture de [tests automatisés](/tests-automatises-php) réelle, en distinguant les tests qui protègent vraiment le métier du décor qui ne teste rien. On scanne les dépendances avec `composer audit`, on identifie le [code mort](/article/code-mort-mission-elimination) qui pollue la lecture, et on repère les anti-patterns structurels.
...
Toutes les imperfections d'une base héritée ne se valent pas. Certaines ralentissent réellement la roadmap ou exposent à un risque, d'autres sont seulement inélégantes et peuvent attendre. Un bon plan de remédiation séquence les chantiers en fonction de leur impact sur votre activité, et intègre les éventuelles montées de version dans un [guide de migration](/article/guide-de-migration-dans-un-projet-symfony) progressif plutôt que dans un grand saut risqué. La référence du [calendrier des versions Symfony](https://symfony.com/releases) sert de boussole pour planifier ces montées sans subir les fins de support.

L'évolution se fait alors de façon incrémentale, chaque livraison s'appuyant sur le filet de tests et la pipeline mis en place à la phase précédente. Vous passez d'un mode défensif, où l'on évite de toucher au code, à un mode offensif, où l'on construit de nouveau de la valeur.

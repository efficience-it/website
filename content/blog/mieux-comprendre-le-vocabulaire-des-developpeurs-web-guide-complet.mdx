...
- Les développeurs sont aussi à l'origine des interfaces de programmation d'applications, aussi appelées **API**. Pour bien les concevoir, il est utile de connaître les [bonnes pratiques des API REST](/article/api-rest-les-bonnes-pratiques). Elles permettent la communication entre le front-end et le back-end, entraînant ainsi l'échange de données et des interactions avec d'autres systèmes.
...
- La **sécurité** du site web est entre leurs mains, cela passe par la gestion de l'authentification des utilisateurs, des sessions et la protection contre les potentielles [vulnérabilités de sécurité (CVE)](/article/cve-comprendre-les-failles-pour-mieux-se-proteger).
...
- Les développeurs effectuent ensuite des **tests** et des **débogages** pour s'assurer que l'application s'affiche et fonctionne correctement sur différents supports et plateformes. Parmi les techniques de débogage informelles, la [méthode du canard en plastique](/article/la-methode-du-canard-en-plastique-le-secret-des-developpeurs) est l'une des plus efficaces et des plus surprenantes.
...
Les tests sont essentiels dans le domaine du développement web. Une stratégie de [tests automatisés en PHP](/tests-automatises-php) bien pensée garantit la fiabilité de chaque livraison. Ils font référence aux processus de vérification et de validation du bon fonctionnement d'un projet.
...
### Quels sont les avantages des conteneurs Docker ?
...
Docker est donc un outil puissant pour créer, déployer et exécuter des applications web dans des conteneurs isolés. Pour aller plus loin sur le sujet, découvrez [pourquoi Docker est indispensable en production aujourd'hui](/article/pourquoi-docker-est-indispensable-en-production-aujourdhui).

### Une migration

Si l'on prend pour exemple Symfony. Celui-ci effectue des mises à jour majeures tous les deux ans, entre-temps, il en existe des mineures et des corrections. Une migration, c'est la transition d'une version à une autre pour un projet. L'idéal étant de limiter les migrations trop importantes pour ne pas avoir de projet obsolète. Pour comprendre comment ces migrations se déroulent en pratique, le [guide de migration dans un projet Symfony](/article/guide-de-migration-dans-un-projet-symfony) donne une méthodologie concrète.
...
- [Pourquoi choisir Symfony pour vos projets](/article/pourquoi-choisir-symfony-pour-vos-projets), Le framework PHP par excellence
- [API REST : les bonnes pratiques](/article/api-rest-les-bonnes-pratiques), Comprendre les APIs qui font communiquer front-end et back-end
- [Tout savoir sur la mise en cache](/article/tout-savoir-sur-la-mise-en-cache-tips), Le cache expliqué en détail
- [Les conventions de codage](/article/coding-conventions), Les règles qui structurent le code source d'un projet

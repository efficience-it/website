...
image: "/images/blog/postman-newman.webp"
updatedAt: "2026-01-13"
howTo:
  name: "Exécuter des tests Postman avec Newman dans GitLab CI"
  description: "Automatiser l'exécution d'une collection Postman dans une pipeline GitLab CI grâce au runner Newman."
  steps:
    - name: "Exporter la collection Postman et l'environnement"
      text: "Depuis Postman, exporter la collection en v2.1 et l'environnement associé, puis les versionner dans le dépôt sous tests/postman/."
    - name: "Ajouter un job Newman dans .gitlab-ci.yml"
      text: "Créer un stage de test qui lance newman run sur la collection exportée avec l'image officielle postman/newman."
    - name: "Injecter les secrets via les variables GitLab"
      text: "Définir les tokens et URLs d'environnement comme variables protégées dans les paramètres CI/CD du projet plutôt que dans le fichier d'environnement."
    - name: "Activer le data-driven testing"
      text: "Passer un fichier CSV ou JSON via l'option --iteration-data pour exécuter la collection sur plusieurs jeux de données."
    - name: "Publier les rapports en artefacts"
      text: "Générer un rapport HTML avec --reporters htmlextra et l'exposer comme artefact GitLab pour faciliter le débogage des échecs."
---

Tester une API manuellement fonctionne au début d'un projet. Mais dès que l'on adopte les [conventions de codage](/article/coding-conventions) en équipe, l'automatisation des tests devient une nécessité. Dès que l'équipe grandit, que les endpoints se multiplient et que les déploiements s'accélèrent, l'absence de tests automatisés devient un risque concret : régressions silencieuses, contrats cassés, intégrations fragiles. Newman, le runner CLI de Postman, permet de transformer des collections de requêtes en suite de tests exécutable dans un pipeline GitLab CI. Les équipes qui versionnent tout dans Git regardent aussi du côté de [Bruno, l'alternative open source à Postman](/article/bruno-alternative-open-source-postman-tester-api), dont les collections sont stockées en fichiers texte. Voici comment mettre cela en place, puis comment aller au-delà d'une configuration basique.
...
La gestion des variables d'environnement est le premier point où la configuration devient intéressante. Comme pour la [gestion des secrets dans un projet Symfony](/article/dbtoolsbundle-anonymiser-vos-bases-de-donnees), une stratégie robuste combine trois niveaux.
...
La qualité d'une suite de tests Postman repose sur la pertinence des assertions, tout comme la qualité du code repose sur un [outillage d'analyse statique comme PHPStan](/article/comment-phpstan-peut-vous-aider-a-ameliorer-la-qualite-de-votre-code-php). Tester uniquement le code HTTP 200 ne suffit pas. Une stratégie d'assertion progressive couvre plusieurs niveaux : le code de retour HTTP, la structure du payload (présence des champs attendus, types corrects), les valeurs métier (un calcul retourne le bon résultat, une liste est triée correctement), et les headers de réponse (Content-Type, Cache-Control, CORS).
...
La mise en cache des dépendances Node.js évite de réinstaller Newman à chaque exécution. Un cache GitLab CI sur le répertoire global npm réduit le `before_script` de 15 secondes à moins de 2. Les runners GitLab CI fonctionnent souvent dans des conteneurs : [pourquoi Docker est indispensable en production](/article/pourquoi-docker-est-indispensable-en-production-aujourdhui) explique comment bien configurer cet environnement d'exécution. Pour des projets plus ambitieux, notre [accompagnement cloud et DevOps](/cloud-et-devops) couvre l'industrialisation complète de ce type de pipeline.
...
Newman et Postman résolvent un problème précis : exécuter des tests fonctionnels HTTP contre une API déployée. Pour que ces tests soient fiables, l'API elle-même doit être bien conçue : [les bonnes pratiques REST](/article/api-rest-les-bonnes-pratiques) définissent des contrats stables qui simplifient l'écriture des assertions. Cette approche a sa place dans la pyramide de tests, mais il est essentiel de comprendre où elle se situe et ce qu'elle ne couvre pas.
...
- [Swagger Nelmio bundle et ses fonctionnalités](/article/swagger-nelmio-bundle-et-ses-fonctionnalites-pourquoi-lutilise-t-on), documenter vos API pour faciliter la création de collections de tests
- [Former vos équipes à la sécurité informatique efficacement](/article/comment-former-vos-equipes-a-la-securite-informatique-en-toute-simplicite), intégrer les tests de sécurité dans vos pipelines CI/CD

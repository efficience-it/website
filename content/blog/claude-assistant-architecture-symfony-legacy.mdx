...
Un hook simple avec [Deptrac](https://github.com/qossmic/deptrac) qui vérifie les dépendances entre couches, en s'appuyant sur le [composant Console de Symfony](https://symfony.com/doc/current/components/console.html) pour l'exécution CLI :
...
Claude reçoit le feedback de PHPStan en temps réel et corrige ses propres erreurs de typage. Le cycle "générer → vérifier → corriger" se fait sans intervention humaine. Pour tirer le meilleur parti de cette intégration, consulter [les 10 erreurs les plus courantes détectées par PHPStan au niveau max sur Symfony](/article/phpstan-niveau-max-symfony-10-erreurs) permet d'affiner les règles à encoder dans le CLAUDE.md.
...
- [Documentation Claude Code](https://docs.anthropic.com/en/docs/claude-code), guide officiel et configuration des fichiers mémoire

...
image: "/images/blog/cover-symfony-moldus.webp"
...
Le conteneur de services est le cœur du framework. Toute classe PHP déclarée comme service est automatiquement instanciée, configurée et injectée là où elle est requise. Ce mécanisme d'injection est au cœur de la philosophie de découplage de Symfony, un principe poussé à l'extrême dans [l'architecture hexagonale où le domaine ne connaît pas le framework](/article/domain-ne-devrait-jamais-connaitre-symfony). Le mécanisme repose sur l'autowiring : Symfony analyse les type-hints des constructeurs pour résoudre les dépendances sans configuration explicite.
...
Le composant Runtime, introduit dans Symfony 5.3, est une évolution architecturale fondamentale souvent sous-estimée. Il découple l'application de son environnement d'exécution. Le même code peut tourner derrière Apache/mod_php, PHP-FPM, un serveur Swoole, [FrankenPHP](/article/concretement-cest-quoi-frankenphp) ou même en mode CLI avec les [commandes invocables](/article/commandes-invocables-symfony-attributs-console), sans modification.

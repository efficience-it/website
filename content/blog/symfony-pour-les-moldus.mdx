...
Le conteneur de services est le coeur du framework. Toute classe PHP declaree comme service est automatiquement instanciee, configuree et injectee la ou elle est requise. Ce mecanisme d'injection est au coeur de la philosophie de decouplage de Symfony, un principe pousse a l'extreme dans [l'architecture hexagonale ou le domaine ne connait pas le framework](/article/domain-ne-devrait-jamais-connaitre-symfony). Le mécanisme repose sur l'autowiring : Symfony analyse les type-hints des constructeurs pour résoudre les dépendances sans configuration explicite.
...
Le composant Runtime, introduit dans Symfony 5.3, est une évolution architecturale fondamentale souvent sous-estimée. Il découple l'application de son environnement d'exécution. Le meme code peut tourner derriere Apache/mod_php, PHP-FPM, un serveur Swoole, [FrankenPHP](/article/concretement-cest-quoi-frankenphp) ou meme en mode CLI, sans modification.

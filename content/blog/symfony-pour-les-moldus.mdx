...
Le conteneur de services est le coeur du framework. Toute classe PHP déclarée comme service est automatiquement instanciée, configurée et injectée là où elle est requise. Ce mécanisme d'injection est au coeur de la philosophie de découplage de Symfony, un principe poussé à l'extrême dans [l'architecture hexagonale où le domaine ne connaît pas le framework](/article/domain-ne-devrait-jamais-connaitre-symfony). Le mécanisme repose sur l'autowiring : Symfony analyse les type-hints des constructeurs pour résoudre les dépendances sans configuration explicite.

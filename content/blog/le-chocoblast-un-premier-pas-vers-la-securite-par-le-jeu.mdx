...
Nous vous invitons à adopter ce jeu ludique, qui permet de sensibiliser toute l'équipe sur les risques de fuite des données. En fait, dès que vous quittez votre poste non verrouillé, cela représente un danger pour l'entreprise car vous avez sur vos ordinateurs un port USB, des fichiers sensibles, des mails, des coordonnées accessibles qui peuvent servir des desseins malveillants. On parle principalement d'espionnage industriel, d'usurpation d'identité, de social hacking, et donne lieu à toute autre sorte de hacks pouvant nuire à vous et à l'entreprise.

Le but de ce jeu est d'amener les collaborateurs à **prendre l'habitude de verrouiller** leurs ordinateurs dès qu'ils ne sont plus devant ou ne peuvent plus le surveiller. Pour mieux comprendre le [vocabulaire technique des développeurs web](/article/mieux-comprendre-le-vocabulaire-des-developpeurs-web-guide-complet), y compris les termes liés à la sécurité, nous avons aussi rédigé un guide complet. C'est pour cela que des règles très précises ont été mises en place.
...
Dans le cadre du social engineering, un attaquant peut exploiter un poste ouvert pour installer un logiciel espion, modifier des paramètres de sécurité ou accéder à des systèmes internes auxquels la victime a des droits. Les conséquences peuvent aller de la fuite de données clients à la compromission complète du réseau de l'entreprise. Pour mieux comprendre comment ces vulnérabilités sont répertoriées et exploitées, il est utile de se pencher sur [les failles CVE et leur fonctionnement](/article/cve-comprendre-les-failles-pour-mieux-se-proteger). Le Chocoblast rend ces risques tangibles en démontrant, de manière inoffensive, à quel point il est facile d'agir sur un poste non protégé.

## Quelques règles

- L'ensemble des collaborateurs doivent être destinataires du message écrit
...
Le Chocoblast installe aussi une **vigilance collective**, un peu comme la [méthode du canard en plastique](/article/la-methode-du-canard-en-plastique-le-secret-des-developpeurs) encourage les développeurs à verbaliser leurs problèmes pour mieux les résoudre. Chaque membre de l'équipe devient un acteur de la sécurité, attentif non seulement à son propre poste mais aussi à celui de ses voisins. Cette responsabilisation partagée dépasse le simple verrouillage d'écran : elle crée un état d'esprit où la sécurité n'est plus perçue comme une contrainte imposée par la direction, mais comme un réflexe naturel porté par le groupe.
...
- [Les conventions de code : un socle pour la qualité](/article/coding-conventions), Poser des règles partagées pour réduire les erreurs et homogénéiser les pratiques
- [Comment se passe un audit chez Efficience IT ?](/article/comment-se-passe-un-audit-chez-efficience-it-quel-contenu-comment-procede-t-on-quels-sont-les-criteres-quel-procede), Comprendre le déroulement d'un audit de sécurité et de qualité de code
- [La dette technique : faut-il vraiment en avoir peur ?](/article/la-dette-technique-faut-il-vraiment-en-avoir-peur), Comprendre pourquoi négliger les bonnes pratiques alimente la dette technique

...
Un point souvent négligé : le `llms.txt` dynamique doit être mis en cache de manière appropriée. Un `Cache-Control` trop agressif empêche les IA de voir les mises à jour, un problème que l'on retrouve aussi dans les [stratégies de mise en cache classiques](/article/tout-savoir-sur-la-mise-en-cache-tips). Aucun cache du tout expose le serveur à des requêtes répétées par des crawlers IA de plus en plus fréquents. Un `max-age` de quelques heures à une journée constitue un bon compromis.

...
howTo:
  ...
  steps:
    - name: "Préparer Doctrine pour PostgreSQL"
      text: "Mettre à jour la DATABASE_URL et l'option server_version dans doctrine.yaml, puis adapter les types Doctrine spécifiques à MySQL (json, datetime, enum)."
    - name: "Générer et valider le schéma cible"
      text: "Lancer doctrine:schema:validate puis doctrine:migrations:diff pour produire la migration PostgreSQL et vérifier la correspondance avec les entités."
    - name: "Migrer les données"
      text: "Utiliser pgloader ou un script applicatif pour copier les données de MySQL vers PostgreSQL en respectant les conversions de type (TINYINT, ENUM, JSON)."
    - name: "Corriger les pièges courants"
      text: "Ajuster les requêtes natives sensibles à la casse, les fonctions GROUP BY strictes et les comportements de timezone propres à PostgreSQL."
    - name: "Basculer la production en dual-write"
      text: "Activer une phase de double écriture sur les deux bases, valider PostgreSQL en lecture, puis couper MySQL une fois la cohérence vérifiée."
faq:

...
Sur un projet Symfony, le tutoriel correspond au parcours d'onboarding d'un nouveau développeur. Pour comprendre pourquoi Symfony est un choix structurant qui mérite d'être bien documenté, notre article sur [les raisons de choisir Symfony pour vos projets](/article/pourquoi-choisir-symfony-pour-vos-projets) pose ce contexte. Il ne s'agit pas de documenter chaque bundle : il s'agit de guider quelqu'un de `git clone` jusqu'à sa première contribution fonctionnelle.
...
Le piège classique : transformer le tutoriel en encyclopédie. Si votre tutoriel d'onboarding dépasse trente minutes, il contient probablement des explications ou de la référence qui appartiennent à d'autres quadrants. Le [parcours en 6 étapes pour monter en compétences sur Symfony](/article/les-6-etapes-pour-monter-en-competences-sur-symfony) illustre cette approche progressive.
...
Les explications sont le quadrant le plus négligé et pourtant le plus précieux pour les développeurs seniors. Elles répondent à la question "pourquoi" : pourquoi le projet utilise Messenger plutôt qu'un système de queue maison, pourquoi l'authentification passe par un custom authenticator plutôt que par le form_login intégré, pourquoi le découpage en bounded contexts suit tel périmètre.

Ce quadrant accueille les ADR (Architecture Decision Records), les analyses de compromis, les retours d'expérience post-incident. Il documente le raisonnement derrière le code, information qui disparaît dès que les développeurs originaux quittent le projet. Le choix entre [monolithe modulaire et micro-services](/article/quelle-architecture-de-projet-choisir-entre-micro-service-ou-monolithe-modulaire) est un exemple typique de décision d'architecture qui mérite une explication dédiée.
...
Stocker la documentation dans le même dépôt que le code (approche docs-as-code), en s'inspirant du [guide de contribution à la documentation Symfony](https://symfony.com/doc/current/contributing/documentation/index.html), permet de lier les modifications de documentation aux pull requests qui modifient le comportement. Ce principe rejoint celui des [conventions de codage](/article/coding-conventions) : un contrat technique versionné, partagé et maintenu par toute l'équipe. Un template de PR qui inclut une section "documentation impactée" rend le réflexe systématique.
...
L'intégration dans la CI, au même titre que [PHPStan](/article/comment-phpstan-peut-vous-aider-a-ameliorer-la-qualite-de-votre-code-php) pour le code, va au-delà de la vérification orthographique. Vérifier que les exemples de code compilent, que les liens internes ne sont pas cassés, que la référence générée est à jour : ces contrôles automatisés réduisent la dette documentaire avant qu'elle ne s'accumule.
...
Chaque équipe peut détenir la responsabilité de la référence de ses propres services tout en contribuant aux guides pratiques transverses. Notre offre d'[accompagnement et conseil](/accompagnement-et-conseil) inclut la mise en place de ces pratiques documentaires au sein des équipes. Les explications architecturales, elles, relèvent naturellement de la responsabilité de l'équipe plateforme ou de l'architecte. Cette répartition des responsabilités évite le problème classique de la documentation orpheline que personne ne maintient.

...
category: "PHP"
excerpt: "Les PSR (PHP Standards Recommendations) sont des normes édictées par la communauté PHP pour établir des standards de codage communs et cohérents."
image: "/images/blog/cover-psr-php.webp"
...
Le PSR-0 (aujourd'hui obsolète) a introduit le concept d'autoloading standardisé en PHP. Le PSR-4, qui l'a remplacé, établit la correspondance entre un namespace et un répertoire de base. C'est le standard que Composer utilise nativement : chaque entrée `autoload` dans `composer.json` repose sur PSR-4. Pour maîtriser les subtilités de cet outil, notre guide sur [l'utilisation de Composer dans le développement Symfony](/article/utilisation-de-composer-dans-le-developpement-symfony-conseils-pratiques) couvre les commandes essentielles. Sans cette convention, l'écosystème de packages PHP tel qu'on le connaît n'existerait tout simplement pas.
...
Le PSR-11 standardise l'injection de dépendances avec deux méthodes : `get(string $id)` et `has(string $id)`. Symfony DI, PHP-DI, Laravel Container, League Container : tous implémentent cette interface. Le [conteneur de services Symfony](https://symfony.com/doc/current/service_container.html) est l'une des implémentations les plus abouties de ce standard. Cela permet à des bibliothèques tierces de consommer un conteneur sans savoir lequel est utilisé.
...
Le PSR-14 a introduit le concept d'événement stoppable via `StoppableEventInterface`, permettant à un listener de court-circuiter la propagation. Ce pattern est particulièrement utile pour les systèmes de validation ou d'autorisation où un rejet doit interrompre la chaîne. Symfony exploite pleinement ce standard, notamment dans [Messenger qui s'appuie sur les événements pour orchestrer l'architecture hexagonale](/article/symfony-messenger-colonne-vertebrale-archi-hexagonale).
...
Les PSR facilitent aussi la [migration entre versions majeures de frameworks](/article/rector-et-ses-pouvoirs-maitrisez-levolution-de-votre-code-symfony), puisque les interfaces standardisées restent stables. Pour une équipe, adopter les PSR signifie aussi réduire la surface de décision : les débats sur le style de code, le choix d'interface de logging ou le format des messages HTTP sont déjà tranchés. L'énergie se concentre sur la logique métier.

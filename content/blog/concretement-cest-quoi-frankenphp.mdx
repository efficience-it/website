...
image: "/images/blog/cover-frankenphp.webp"
proficiencyLevel: "Expert"
updatedAt: "2026-01-13"
faq:
  - question: "FrankenPHP est-il prêt pour la production ?"
    answer: "Oui, FrankenPHP est stable et utilisé en production. Il est officiellement supporté par Symfony et fait partie de la distribution Docker officielle de PHP. Les gains de performance sont significatifs grâce au mode worker qui évite le bootstrap PHP à chaque requête."
  - question: "Quelle est la différence entre FrankenPHP et PHP-FPM ?"
    answer: "PHP-FPM utilise le protocole FastCGI pour communiquer entre le serveur web (Nginx) et PHP, ce qui ajoute un overhead à chaque requête. FrankenPHP embarque directement l'interpréteur PHP dans un serveur Caddy en Go, éliminant cette couche intermédiaire. Le mode worker garde l'application en mémoire entre les requêtes."
  - question: "FrankenPHP fonctionne-t-il avec Symfony et Laravel ?"
    answer: "Oui, FrankenPHP est compatible avec tous les frameworks PHP. Symfony le supporte officiellement via le composant Runtime. Laravel fonctionne également avec FrankenPHP grâce à Laravel Octane. La configuration est minimale : une seule ligne dans le Dockerfile suffit."
---
...
3. La réponse est retournée, l'état de la requête est réinitialisé, les variables et objets sont libérés
...
À cette charge computationnelle s'ajoute une complexité opérationnelle. L'architecture de production classique requiert la coordination de plusieurs services : Nginx ou Apache en reverse proxy, PHP-FPM pour l'exécution, Certbot ou un autre outil pour la gestion des certificats TLS, parfois Varnish pour le cache HTTP. Les stratégies de mise en cache sont abordées en détail dans notre guide sur [la mise en cache appliquée aux applications PHP](/article/tout-savoir-sur-la-mise-en-cache-tips). Chaque service apporte son fichier de configuration, ses logs, ses mécanismes de reload et ses propres modes de défaillance. Cette recherche de simplicité d'exécution rejoint le débat entre [WASM et container côté serveur](/article/wasm-ou-container-que-choisir-en-2026).
...
Le résultat concret : un unique binaire capable de servir une application PHP avec support natif de HTTP/1.1, HTTP/2 et HTTP/3, génération automatique de certificats TLS via Let's Encrypt ou ZeroSSL, et support du temps réel via le protocole Mercure. Cette simplification du runtime est un atout pour l'[hébergement Symfony](/hebergement-symfony) : un seul processus remplace toute une stack de services à configurer et maintenir. FrankenPHP peut déployer nativement Symfony, Laravel, API Platform, WordPress, Drupal ou toute application PHP standard. Combiné avec [Docker en production](/article/pourquoi-docker-est-indispensable-en-production-aujourdhui), le binaire unique simplifie considérablement l'orchestration des conteneurs.
...
Chaque requête PHP est traitée dans une goroutine Go dédiée. FrankenPHP nécessite une version ZTS (Zend Thread Safety) de PHP pour garantir la sécurité des accès concurrents à l'interpréteur. Le scheduler de Go gère l'ordonnancement de ces goroutines sur les threads système disponibles, ce qui permet de traiter un grand nombre de requêtes concurrentes avec une empreinte mémoire maîtrisée. Cette architecture élimine le surcoût du protocole FastCGI et les context switches entre les processus Nginx et PHP-FPM.
...
FrankenPHP s'intègre particulièrement bien avec Symfony, un framework dont [les atouts pour les projets web](/article/pourquoi-choisir-symfony-pour-vos-projets) se combinent naturellement avec les gains de performance du mode worker. Pour activer le mode worker avec Symfony, l'intégration passe par le [composant Runtime](https://symfony.com/doc/current/components/runtime.html) :
...
La troisième étape est le déploiement en production avec monitoring renforcé. Pour les équipes qui envisagent une [architecture micro-services ou monolithe modulaire](/article/quelle-architecture-de-projet-choisir-entre-micro-service-ou-monolithe-modulaire), FrankenPHP simplifie le déploiement dans les deux cas. Les images Docker officielles de FrankenPHP supportent les health checks natifs de Caddy, le graceful shutdown pour les rolling updates Kubernetes, et la configuration par variables d'environnement.

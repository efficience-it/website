...
event:
  ...
  endDate: "2024-10-11"
  location:
    name: "Hôtel New York – The Art of Marvel, Disneyland Paris"
    address: "Allée de la Belle Étoile, 77700 Marne-la-Vallée, France"
  organizer:
    name: "AFUP – Association Française des Utilisateurs de PHP"
    url: "https://afup.org"
  url: "https://event.afup.org/forum-php-2024/"
---
...
Les objets paresseux (lazy objects) représentent l'une des fonctionnalités les plus attendues de PHP 8.4. Cette évolution du langage s'inscrit dans la trajectoire décrite dans notre article sur les [contributions de la Fondation PHP](/article/la-fondation-php-souffle-ses-trois-bougies). Ils permettent de différer l'initialisation d'un objet jusqu'à ce qu'il soit réellement utilisé, optimisant la gestion de la mémoire et les performances. Concrètement, un objet paresseux se comporte comme un proxy transparent : il n'exécute son constructeur et n'alloue ses ressources qu'au moment du premier accès à l'une de ses propriétés ou méthodes.
...
Simon André a présenté les avancées de l'initiative Symfony UX, qui vise à combler le fossé entre le développement back-end en PHP et les interfaces front-end modernes, qu'elles s'appuient sur Symfony UX ou sur un framework comme [Vue.js](/developpement-vuejs). Deux nouveaux composants ont été introduits : **Icon**, qui offre une gestion simplifiée des icônes SVG avec un système de cache et de lazy loading intégré, et **Map**, qui permet d'afficher des cartes interactives sans dépendre d'un framework JavaScript lourd.
...
Michelle Sanver a présenté des pratiques concrètes pour rendre les applications Symfony plus respectueuses de l'environnement, un sujet que nous explorons dans notre article sur [l'éco-conception web](/article/eco-conception-un-ideal-en-marche-ou-une-illusion-durable). Son talk abordait la réduction des ressources serveur inutiles, l'optimisation des requêtes en base de données pour limiter la consommation énergétique, et l'adoption de stratégies de mise en cache intelligentes. Elle a rappelé que le numérique représente environ 4 % des émissions mondiales de gaz à effet de serre et que chaque optimisation, même modeste, compte à l'échelle de millions d'utilisateurs.

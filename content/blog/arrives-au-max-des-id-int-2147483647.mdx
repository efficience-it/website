...
Dans tous les cas, **toute procédure doit être testée sur un environnement miroir de puissance équivalente à la production**, en utilisant par exemple [DbToolsBundle pour anonymiser vos données de test](/article/dbtoolsbundle-anonymiser-vos-bases-de-donnees).
...
Les ingénieurs [Cloud et DevOps](/cloud-et-devops) de l'agence accompagnent les entreprises dans la sécurisation de leurs bases MySQL et MariaDB, avec des méthodes validées sur des environnements de production à fort trafic. Ce type d'intervention s'inscrit souvent dans un chantier plus large de [modernisation d'application PHP](/modernisation-application-php).

...
category: "IA"
excerpt: "Comment connecter un LLM à votre base Doctrine grâce au RAG, Symfony AI et pgvector. Indexer, rechercher et interroger vos données métier."
image: "/images/blog/rag-symfony.webp"
proficiencyLevel: "Expert"
---

Vous avez une application Symfony en production, une base [Doctrine](/article/doctrine-orm-3-0-une-nouvelle-version-majeure-pour-les-bases-de-donnees) bien remplie, et vous vous demandez comment rendre tout ça interrogeable par un LLM. Pas via une API REST classique. Via du langage naturel : « quels tickets similaires ont déjà été résolus ? », « quel client a eu ce problème en janvier ? ».
...
Si vous avez besoin de performances poussées sur de gros volumes ou de fonctionnalités avancées (filtrage par payload, sharding), Qdrant est un excellent choix. C'est un vector store dédié, conçu spécifiquement pour la recherche de similarité. Pour la recherche full-text classique qui accompagne souvent une recherche sémantique, notre [comparatif entre Elasticsearch et Algolia](/article/elasticsearch-ou-algolia-moteur-de-recherche-symfony) détaille les arbitrages entre un moteur auto-hébergé et une solution SaaS.
...
Dans une application Symfony sérieuse, vous ne voulez pas coupler votre domaine à un vector store spécifique. C'est le principe fondamental de [l'architecture hexagonale appliquée à Symfony](/article/symfony-messenger-colonne-vertebrale-archi-hexagonale) : isoler le domaine de l'infrastructure. On définit un port :

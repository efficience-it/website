...
Ce choix impose PHP 8.2 comme version minimale et aligne Doctrine sur les standards modernes du langage. [Rector peut automatiser cette migration](/article/rector-et-ses-pouvoirs-maitrisez-levolution-de-votre-code-symfony) des annotations vers les attributs PHP 8. Pour les projets encore en annotations, cette migration représente le premier chantier à anticiper.

### Suppression des méthodes dépréciées de l'EntityManager

La méthode `merge()` disparaît de l'`EntityManager`. Ces méthodes, sources fréquentes de bugs subtils liés à l'identity map, avaient été dépréciées depuis plusieurs versions mineures. Leur suppression force l'adoption de patterns plus explicites pour la gestion du cycle de vie des entités.
...
Cette évolution transforme la détection d'erreurs. Là où un bug de typage dans une relation `OneToMany` ne se manifestait qu'à l'exécution, [PHPStan](/article/comment-phpstan-peut-vous-aider-a-ameliorer-la-qualite-de-votre-code-php) et Psalm peuvent désormais le signaler dès l'analyse. Pour les équipes qui maintiennent des modèles de domaine complexes avec des dizaines de relations, le gain en fiabilité est substantiel.
...
Le mécanisme de proxies, essentiel au lazy loading, repose désormais sur les lazy ghost objects (via `symfony/var-exporter`). Ce nouveau mécanisme remplace l'ancien système basé sur l'héritage de classes et réduit la surface de configuration tout en améliorant la gestion mémoire lors du chargement d'entités.

Les lazy ghost objects rendent la pré-génération des proxies obsolète. La commande `orm:generate-proxies` a été supprimée dans ORM 3.0, car le nouveau mécanisme ne nécessite plus de fichiers de proxies générés à l'avance.

### Stratégies de génération d'identifiants

Avec DBAL 4, la stratégie `SEQUENCE` est abandonnée au profit de `IDENTITY` (basée sur `GENERATED BY DEFAULT AS IDENTITY`), qui devient le standard pour [PostgreSQL dans les projets Symfony](/base-de-donnees-postgresql-symfony). La stratégie `AUTO` sélectionne automatiquement l'approche optimale selon le SGBD. Les UUID et ULID sont mieux supportés comme [identifiants de premier plan](/article/prendre-int-uuid-ou-ulid-pour-un-index-de-base-de-donnees), ce qui facilite les architectures distribuées où les identifiants séquentiels posent des problèmes de collision. Si votre projet utilise encore des colonnes `INT` et approche leurs limites, l'article sur [la migration des ID INT vers BIGINT](/article/arrives-au-max-des-id-int-2147483647) détaille la procédure sans interruption de service.
...
Le nouveau système de proxies contribue également à cette amélioration. Les lazy ghost objects fonctionnent sans fichiers de proxies générés sur le disque, ce qui supprime les I/O associées et simplifie le déploiement en production.
...
Doctrine ORM 3.0 marque un tournant dans la maturité de l'écosystème de persistance PHP. En supprimant le code hérité, en renforçant le typage et en s'alignant sur les standards PSR, cette version pose les fondations d'une bibliothèque plus solides, plus performante et plus maintenable. Couplée à DBAL 4, elle impose une réflexion architecturale globale sur la couche de persistance. La migration demande de la rigueur, mais le résultat est un socle technique modernisé, prêt à supporter les exigences des applications PHP des prochaines années.

Faites appel à nos équipes pour vous épauler dans cette migration. Notre service de [développement web sur mesure](/developpement-web-sur-mesure) inclut l'accompagnement sur les montées de version Doctrine.

...
category: "Symfony"
excerpt: "Symfony est reconnu comme un framework PHP solide et flexible. Les développeurs ont fréquemment besoin de données fictives pour les tests."
...
App\Entity\User:
    user_{1..20}:
        ...
        lastName: '<lastName()>'
        email: '<email()>'
        password: '$2y$13$fixedHashForFixtures'
        roles: ['ROLE_USER']
        createdAt: '<dateTimeBetween("-1 year", "now")>'
```

Le mot de passe est ici un hash bcrypt pré-calculé. Alice ne fournit pas de formatter natif pour hasher les mots de passe. En pratique, utilisez un processor Alice qui injecte le `UserPasswordHasherInterface` de Symfony pour hasher dynamiquement les mots de passe au moment du chargement des fixtures.
...
Pour garantir la cohérence des données, le choix du [type d'identifiant (INT, UUID ou ULID)](/article/prendre-int-uuid-ou-ulid-pour-un-index-de-base-de-donnees) impacte directement la génération des fixtures et les performances en test. Le trait `ResetDatabase` isole chaque test PHPUnit avec un rollback transactionnel automatique, en s'appuyant sur les principes décrits dans la [documentation officielle de Symfony sur les tests](https://symfony.com/doc/current/testing.html), ce qui élimine les effets de bord entre tests sans le coût d'un rechargement complet des fixtures :

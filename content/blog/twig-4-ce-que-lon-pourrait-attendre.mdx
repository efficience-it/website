...
L'intérêt dépasse la simple documentation. Un tel tag ouvrirait la porte à l'analyse statique par [PHPStan](/article/comment-phpstan-peut-vous-aider-a-ameliorer-la-qualite-de-votre-code-php) ou Psalm : détection de propriétés inexistantes, de types incompatibles, de variables non passées. Le template deviendrait un artefact vérifiable dans la chaîne CI, au même titre qu'une classe PHP. Pour les équipes qui maintiennent des centaines de templates, le gain en fiabilité serait considérable.
...
Les fonctions fléchées existent déjà en Twig 3 (depuis la version 3.0) avec les filtres `|filter` et `|map`. Twig 4 pourrait étendre leur usage et améliorer leur expressivité, notamment pour la composition de filtres :
...
L'opérateur de coalescence `??` existe depuis Twig 3.14. Twig 4 pourrait l'étendre avec des comportements supplémentaires ou une intégration plus profonde dans le moteur :
...
Mais la véritable rupture se situe ailleurs. L'écosystème Symfony UX a introduit les Twig Components avec une syntaxe HTML déclarative qui pourrait être intégrée plus profondément dans le moteur. Ces composants font d'ailleurs partie des [bundles les plus utilisés dans les projets Symfony](/article/les-bundles-les-plus-utilises-dans-les-projets-symfony) modernes :
...
Pour les architectes qui conçoivent des applications Symfony, Twig 4 confirmerait une tendance : le front-end Symfony n'est pas un front-end au rabais, mais un front-end opiniâtre, qui assume le serveur comme point de contrôle principal et traite le JavaScript comme un enrichissement progressif. Pour comprendre pourquoi cette approche s'inscrit dans la philosophie du framework, l'article [pourquoi choisir Symfony pour vos projets](/article/pourquoi-choisir-symfony-pour-vos-projets) en expose les fondements. Ces évolutions enrichissent directement les projets de [développement web sur mesure](/developpement-web-sur-mesure) que nous réalisons.
...
- [Documentation officielle de Twig](https://twig.symfony.com/doc/3.x/), référence du moteur de templates

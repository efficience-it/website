...
excerpt: "PHPStan est un outil de vérification statique pour PHP. Il examine votre code à la recherche d'erreurs potentielles et améliore la qualité du code."
image: "/images/blog/phpstan-qualite.webp"
faq:
  - question: "À quoi sert PHPStan dans un projet PHP ?"
...
PHP est un langage interprété. Contrairement à Java ou Go, aucune phase de compilation ne vérifie la cohérence des types avant l'exécution. Pendant des années, les développeurs PHP ont découvert leurs erreurs de typage en production, à travers des `TypeError` ou des `null` inattendus, là où [TypeScript](/developpement-typescript) apporte cette rigueur de typage côté JavaScript. L'analyse statique comble ce vide : elle simule une compilation en parcourant l'AST (Abstract Syntax Tree) du code source, sans jamais l'exécuter.
...
PHPStan excelle aussi à repérer le [code mort dans vos projets](/article/code-mort-mission-elimination), ces branches jamais atteintes qui alourdissent la base de code. Coupler PHPStan avec des [tests automatisés](/tests-automatises-php) garantit une couverture maximale des erreurs, tant structurelles que comportementales.
...
Un architecte qui configure PHPStan avec des règles custom, des generics et un niveau 9 ne cherche pas à satisfaire un outil. Il encode les invariants de son système dans un vérificateur automatique qui tourne à chaque commit. C'est de la gouvernance technique exécutable. Notre service d'[analyse de code PHP](/audit-code-php) s'appuie sur ce type d'outillage pour identifier les failles structurelles d'un projet, première étape d'un parcours de [modernisation applicative](/modernisation-applicative).

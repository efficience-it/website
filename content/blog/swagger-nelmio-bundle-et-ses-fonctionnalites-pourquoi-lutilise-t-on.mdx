...
NelmioApiDocBundle, référencé dans la [documentation Symfony sur la construction d'API](https://symfony.com/doc/current/the-fast-track/en/26-api.html), est le pont entre l'écosystème Symfony et cette spécification. Il introspecte les routes, les contrôleurs, les Form Types et les entités Doctrine pour produire un fichier OpenAPI sans écrire une ligne de YAML à la main. Ce bundle est un outil incontournable pour le [developpement d'API Symfony](/api-sur-mesure-symfony) sur mesure, là où l'écosystème Node.js mise sur des frameworks structurés comme [NestJS pour documenter ses API](/api-nodejs-nestjs). Pour tirer pleinement parti de ce bundle, il est utile de maîtriser les [bonnes pratiques des API REST](/article/api-rest-les-bonnes-pratiques) : structure des endpoints, gestion des codes HTTP et cohérence des réponses.
...
Le véritable gain de productivité vient de la documentation automatique des modèles. Le bundle introspecte les propriétés d'une classe PHP pour en déduire le schéma JSON :
...
NelmioApiDocBundle est la première brique de cette chaîne dans l'écosystème Symfony. Son adoption est triviale, mais son potentiel se révèle pleinement quand il s'intègre dans une stratégie de gouvernance API plus large : areas par audience, sécurité par défaut, validation automatisée et génération de SDK. C'est un outil que nous mettons en place systématiquement dans nos projets de [développement web sur mesure](/developpement-web-sur-mesure).

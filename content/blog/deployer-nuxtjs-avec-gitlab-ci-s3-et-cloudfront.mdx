...
howTo:
  name: "Déployer Nuxt.js avec GitLab CI, S3 et CloudFront"
  description: "Publier un site Nuxt.js sur AWS S3 avec distribution CloudFront, automatisé via une pipeline GitLab CI."
  steps:
    - name: "Préparer le socle AWS S3 et CloudFront"
      text: "Créer un bucket S3 pour héberger les fichiers statiques et configurer une distribution CloudFront en amont, avec un utilisateur IAM dédié pour GitLab CI."
    - name: "Construire la pipeline GitLab CI"
      text: "Écrire un .gitlab-ci.yml qui installe les dépendances, génère la version statique avec nuxt generate, puis synchronise le dossier dist sur S3."
    - name: "Configurer le déploiement multi-environnement"
      text: "Définir des règles only/except par branche pour pousser sur des buckets distincts (preview, staging, production) selon le contexte."
    - name: "Mettre en place l'invalidation de cache CloudFront"
      text: "Déclencher une invalidation CloudFront en fin de pipeline pour purger les fichiers HTML modifiés tout en conservant le cache long sur les assets fingerprintés."
---
...
Le principe du moindre privilège s'applique ici. Créez un utilisateur IAM dédié avec uniquement deux permissions : `AmazonS3FullAccess` pour la synchronisation des fichiers et une politique IAM personnalisée avec l'action `cloudfront:CreateInvalidation` pour la purge du cache. Générez une clé d'accès programmatique qui alimentera les variables `AWS_ACCESS_KEY_ID` et `AWS_SECRET_ACCESS_KEY` dans GitLab.
...
Le cache sur `node_modules/` est essentiel. Sans lui, chaque pipeline réinstalle l'intégralité des dépendances, un problème que l'on retrouve aussi lors de la [gestion des dépendances avec Composer](/article/utilisation-de-composer-dans-le-developpement-symfony-conseils-pratiques). Sur un projet Nuxt.js typique, cela représente facilement deux à trois minutes économisées par exécution. À l'échelle d'une équipe qui pousse régulièrement, le gain cumulé est considérable.
...
Dans GitLab, sous Settings > CI/CD > Variables, injectez les quatre secrets : `AWS_CLOUDFRONT_INVALIDATION_ID`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` et `AWS_BUCKET_NAME`. Marquez-les comme protégées et masquées pour éviter toute fuite dans les logs de pipeline. La gestion rigoureuse des secrets fait partie des bonnes pratiques pour [se prémunir contre les failles de sécurité](/article/cve-comprendre-les-failles-pour-mieux-se-proteger).
...
Cette approche transforme radicalement le workflow : créer un nouvel environnement de preview par merge request ne nécessite plus d'intervention manuelle dans la console AWS, mais un simple `terraform apply` paramétré par le nom de la branche. Ce type d'automatisation fait partie de notre offre [Cloud et DevOps](/cloud-et-devops) pour industrialiser les pipelines de déploiement. Pour les applications Symfony, notre offre d'[hébergement Symfony](/hebergement-symfony) intègre ces bonnes pratiques de déploiement automatisé dès la mise en place de l'infrastructure.
...
- [Développement frontend sur mesure](/developpement-frontend), notre offre React, Vue.js et TypeScript

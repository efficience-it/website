...
Les headers standardisés `RateLimit-Limit`, `RateLimit-Remaining` et `RateLimit-Reset` (draft IETF) informent le consommateur de son quota en temps réel. La protection de l'API passe aussi par une bonne compréhension des [failles CVE](/article/cve-comprendre-les-failles-pour-mieux-se-proteger) qui peuvent affecter les dépendances utilisées, et par une validation systématique des contrats à l'aide d'un [client de test d'API comme Bruno](/article/bruno-alternative-open-source-postman-tester-api). Lorsque le quota est dépassé, un `429 Too Many Requests` avec un header `Retry-After` indique quand le client peut relancer.
...
Le pattern Backend for Frontend (BFF) consiste à déployer une gateway spécialisée par type de client, souvent développée en [Node.js](/developpement-nodejs). Le choix entre [micro-service et monolithe modulaire](/article/quelle-architecture-de-projet-choisir-entre-micro-service-ou-monolithe-modulaire) influence directement la manière dont cette gateway est implémentée. L'application mobile reçoit des payloads optimisés et agrégés, tandis que le backoffice web accède à des endpoints plus granulaires. Chaque BFF adapte les réponses sans polluer l'API interne avec des préoccupations de présentation.

...
faq:
  - question: "Quelles sont les bonnes pratiques pour concevoir une API REST ?"
    answer: "Les fondamentaux sont : utiliser les bons verbes HTTP (GET, POST, PUT, DELETE), nommer les ressources au pluriel dans les URIs, retourner les codes de statut HTTP appropriate (200, 201, 404, 422), paginer les collections et versionner l'API. La documentation automatique avec OpenAPI/Swagger est egalement essentielle."
  - question: "Comment securiser une API REST en PHP ?"
    answer: "Les mecanismes de securite essentiels sont l'authentification par token (JWT ou OAuth2), le rate limiting pour prevenir les abus, la validation stricte des entrees, le chiffrement HTTPS obligatoire et la gestion fine des autorisations par role. En Symfony, le composant Security et API Platform couvrent ces besoins nativement."
  - question: "REST ou GraphQL : lequel choisir pour mon API ?"
    answer: "REST convient a la majorite des projets grace a sa simplicite et son ecosysteme mature (cache HTTP, documentation OpenAPI). GraphQL est plus adapte quand les clients ont des besoins de donnees tres variables (applications mobiles, dashboards complexes). Les deux peuvent coexister dans un meme projet."
---

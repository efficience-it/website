...
Le principe fondamental de [l'architecture hexagonale](https://alistair.cockburn.us/hexagonal-architecture/) tient en une phrase : le domaine est au centre, et il ne dépend de rien.
...
[Le composant Validator de Symfony](https://symfony.com/doc/current/validation.html) est puissant, mais il n'a rien à faire dans le domaine.
...
La validation dans le domaine est native. Le [Value Object](https://martinfowler.com/bliki/ValueObject.html) garantit sa propre cohérence à la construction. Pas besoin d'un framework de validation pour ça. Un `Email` invalide ne peut tout simplement pas exister.
...
Ton événement domaine hérite d'une classe Symfony. Si demain tu passes sur [Laravel](https://laravel.com/) ou un autre framework, tu dois réécrire tous tes événements.
...
Le mécanisme qui rend tout ça possible, c'est le [port](https://herbertograca.com/2017/09/14/ports-adapters-architecture/). Une interface définie dans le domaine, implémentée dans l'infrastructure.
...
[Symfony autowire](https://symfony.com/doc/current/service_container/autowiring.html) les interfaces automatiquement quand il n'y a qu'une seule implémentation. Zéro config nécessaire.
...
La discipline humaine ne suffit pas. Sur un projet avec dix développeurs, quelqu'un finira par ajouter un `use Doctrine\*` dans le domaine. [Deptrac](https://github.com/qossmic/deptrac) automatise la vérification.
...
- [Matthias Noback : Advanced Web Application Architecture](https://matthiasnoback.nl/book/advanced-web-application-architecture/), référence sur l'archi hexagonale en PHP
- [Symfony best practices](https://symfony.com/doc/current/best_practices.html), les conventions officielles Symfony
- [Martin Fowler : Bounded Context](https://martinfowler.com/bliki/BoundedContext.html), comprendre les limites du domaine

...
image: "/images/blog/domain-symfony.webp"
...
Cette classe dépend de trois composants Symfony/Doctrine. Certes, les attributs PHP sont des métadonnées inertes : tu peux instancier cette classe et tester `calculateTotal()` sans que Doctrine soit chargé. Mais la dépendance existe dans le code. Tes `use` pointent vers `vendor/`, ton domaine ne compile plus sans ces paquets, et chaque montée de version Doctrine ou Symfony peut casser tes modèles métier. Si tu veux réutiliser cette logique dans un contexte non-Symfony (une commande CLI, un worker, un autre projet), tu traînes tout le framework avec toi.
...
L'événement domaine est un simple objet immutable avec des données. [Symfony Messenger](/article/symfony-messenger-colonne-vertebrale-archi-hexagonale) excelle dans ce rôle de dispatcher d'événements domaine en les acheminant vers les bons handlers via le message bus. Un adaptateur dans l'infrastructure le traduit en événement Symfony pour le dispatcher :
...
Symfony autowire les interfaces automatiquement quand il n'y a qu'une seule implémentation, comme le décrivent les [bonnes pratiques officielles Symfony](https://symfony.com/doc/current/best_practices.html). Zéro config nécessaire.
...
La règle `Domain: []` est la plus importante : le domaine ne dépend de rien. Aucune autre couche n'est autorisée. Si un développeur ajoute un import Doctrine dans le domaine, Deptrac casse le build. Combiné avec [PHPStan comme gardien d'architecture](/article/comment-phpstan-peut-vous-aider-a-ameliorer-la-qualite-de-votre-code-php), tu vérifies à la fois les dépendances entre couches et la cohérence des types.
...
C'est un filet de sécurité permanent. L'architecture n'est plus une convention orale, c'est une règle vérifiée à chaque commit. Coupler Deptrac avec des [conventions de codage formalisées](/article/coding-conventions) dans l'équipe renforce encore davantage l'isolation du domaine sur le long terme.

## Quand ne pas le faire

Cette approche a un coût. Pour un CRUD d'admin avec cinq entités et aucune logique métier, séparer domaine et infrastructure est de l'over-engineering. Le couplage Doctrine n'est un problème que quand il y a de la logique métier à protéger. La question de [choisir entre micro-services et monolithe modulaire](/article/quelle-architecture-de-projet-choisir-entre-micro-service-ou-monolithe-modulaire) se pose dans le même esprit : l'architecture doit servir le projet, pas l'inverse.
...
C'est pour ça que ton domaine ne devrait jamais connaître Symfony. Pas par purisme architectural, pas pour faire joli sur un schéma. Pour que le code le plus important de ton application soit aussi le plus durable. Si tu veux structurer ton projet selon ces principes, notre accompagnement en [architecture hexagonale Symfony](/architecture-hexagonale-symfony) couvre exactement ce type de migration.

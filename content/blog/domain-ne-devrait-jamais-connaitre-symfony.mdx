---
title: "Pourquoi votre Domain ne devrait jamais connaître Symfony"
date: "2026-03-09"
updatedAt: "2026-03-26"
author: "Louis-Arnaud Catoire"
category: "Architecture"
excerpt: "Votre domaine métier ne devrait dépendre de rien. Ni de Symfony, ni de Doctrine. Voici pourquoi et ce que ça change concrètement sur un vrai projet."
image: "/images/blog/domain-symfony.webp"
proficiencyLevel: "Expert"
---

Ouvrez le répertoire `src/Entity/` de n'importe quel projet Symfony. Vous y trouverez des classes avec des attributs Doctrine, des validations Symfony, parfois des appels à des services, et au milieu de tout ça, de la logique métier. Le modèle de commande calcule ses totaux entre deux annotations `#[ORM\Column]`. L'entité utilisateur vérifie les permissions au milieu des `#[Assert\NotBlank]`.
...
- Des classes de votre propre domaine
...
- **Des tests unitaires rapides** : votre domaine se teste sans base de données, sans container, en millisecondes
- **La liberté de changer d'ORM** : improbable mais possible, et surtout ça simplifie les montées de version Doctrine
- **Un domaine lisible** : vos modèles métier ne sont pas pollués par des annotations techniques
- **Des refactorisations sûres** : vous changez la structure de la base sans toucher au domaine, et inversement

## Vérifiez vos dépendances avec Deptrac
...
La règle `Domain: []` est la plus importante : le domaine ne dépend de rien. Aucune autre couche n'est autorisée. Si un développeur ajoute un import Doctrine dans le domaine, Deptrac casse le build. Combiné avec [PHPStan comme gardien d'architecture](/article/comment-phpstan-peut-vous-aider-a-ameliorer-la-qualite-de-votre-code-php), vous vérifiez à la fois les dépendances entre couches et la cohérence des types.
...
C'est pour ça que votre domaine ne devrait jamais connaître Symfony. Pas par purisme architectural, pas pour faire joli sur un schéma. Pour que le code le plus important de votre application soit aussi le plus durable. Si vous voulez structurer votre projet selon ces principes, notre accompagnement en [architecture hexagonale Symfony](/architecture-hexagonale-symfony) couvre exactement ce type de migration.

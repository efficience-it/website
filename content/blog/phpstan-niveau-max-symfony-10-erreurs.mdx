...
Sur un projet de taille moyenne, compte entre 50 et 200 occurrences. Un bon regex dans ton IDE règle ça en une heure. Les évolutions de [Doctrine ORM 3.0](/article/doctrine-orm-3-0-une-nouvelle-version-majeure-pour-les-bases-de-donnees) améliorent d'ailleurs le typage natif des collections.
...
Ne fais jamais de `assert($user !== null)` sauf dans les tests. C'est un pansement qui masque un vrai problème de gestion d'erreur. Respecter les [bonnes pratiques des API REST](/article/api-rest-les-bonnes-pratiques) impose de gérer proprement ces cas de retour `null`.

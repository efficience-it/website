...
excerpt: "Symfony Messenger ne sert pas qu'à envoyer des messages en asynchrone. Découvrez comment l'utiliser comme Command Bus, Query Bus et Event Bus."
...
Un command ne retourne rien. Si vous avez besoin de l'ID de l'entité créée, générez-le **avant** le dispatch (avec un [UUID ou ULID](/article/prendre-int-uuid-ou-ulid-pour-un-index-de-base-de-donnees) par exemple) et passez-le dans le command.
...
Le pattern CQRS/Event-driven avec Messenger est un excellent point de départ pour structurer des applications Symfony maintenables. Si vous voulez approfondir le sujet, la [documentation officielle de Symfony Messenger](https://symfony.com/doc/current/messenger.html) est complète et bien structurée, et le [dépôt GitHub du composant](https://github.com/symfony/messenger) permet de lire le code source pour comprendre les internals.

L'architecture hexagonale avec Messenger n'est pas une mode. C'est une approche éprouvée qui rend votre code testable, maintenable et évolutif. Notre accompagnement en [architecture hexagonale Symfony](/architecture-hexagonale-symfony) aide les équipes à mettre en place ces patterns sur des projets existants. Le framework devient un détail d'implémentation. Le domaine reste pur. Et vos collègues vous remercieront quand ils ouvriront le projet dans six mois. Si vous partez d'un projet existant et que vous voulez adopter cette architecture progressivement, le [retour de mission sur la migration vers l'architecture hexagonale](/article/migration-symfony-architecture-hexagonale-retour-mission) offre une démarche pragmatique issue du terrain. Côté API, notre offre d'[architecture asynchrone Symfony](/api-sur-mesure-symfony) prend le relais pour exposer commands et events à des consommateurs externes, et la [conception d'une application métier sur mesure](/developpement-web-sur-mesure) décrit comment articuler ces briques dans un produit livré.

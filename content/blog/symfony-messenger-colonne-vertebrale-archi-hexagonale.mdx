...
image: "/images/blog/symfony-messenger.webp"
...
Un bus de messages, c'est un médiateur entre celui qui envoie une intention et celui qui la traite. Le sender ne connaît pas le handler. Le handler ne sait pas d'où vient le message. Ce découplage est exactement le fondement d'une architecture hexagonale : [le domaine ne devrait jamais connaître Symfony](/article/domain-ne-devrait-jamais-connaitre-symfony), c'est l'infrastructure qui s'adapte.
...
La [documentation officielle du composant Messenger](https://symfony.com/doc/current/messenger.html) sur les bus multiples explique comment déclarer plusieurs bus. Voici la configuration de base dans `messenger.yaml` :
...
Le handler dépend de **ports** (interfaces) du domaine, pas d'implémentations concrètes. `OrderRepositoryInterface` est défini dans le domaine. L'implémentation [Doctrine](/article/doctrine-orm-3-0-une-nouvelle-version-majeure-pour-les-bases-de-donnees) est dans l'infrastructure. Le handler ne sait pas et n'a pas besoin de savoir comment la persistence fonctionne.
...
Par défaut, les events devraient être **asynchrones**. C'est le cas d'usage naturel, et c'est exactement ce qui rend Messenger indispensable pour les [applications industrielles](/secteur/industrie) qui orchestrent des workflows complexes entre plusieurs systèmes : la validation de la commande n'a pas besoin d'attendre que l'email soit envoyé. Configure le routing dans `messenger.yaml` :
...
C'est un point crucial que beaucoup ratent. Tes commands et queries ne doivent **jamais** dépendre de Symfony. Messenger n'impose ni attribut, ni interface sur les messages. Ce sont des objets du domaine, des DTOs purs.
...
Tu connais ce `OrderService` avec `createOrder()`, `validateOrder()`, `cancelOrder()`, `getOrderById()`, `getOrdersByCustomer()`, `calculateTotal()`, et 25 autres méthodes ? Ce service god-class qui viole le Single Responsibility Principle, accumule de la [dette technique](/article/la-dette-technique-faut-il-vraiment-en-avoir-peur) et qui est impossible à tester proprement ?
...
Pas de kernel Symfony, pas de container, pas de base de données. Un test unitaire pur qui s'exécute en millisecondes. C'est le bénéfice direct d'avoir des handlers qui dépendent d'abstractions, et une stratégie de [tests automatisés](/tests-automatises-php) bien pensée couvre naturellement ces handlers.
...
Un command ne retourne rien. Si tu as besoin de l'ID de l'entité créée, génère-le **avant** le dispatch (avec un [UUID ou ULID](/article/prendre-int-uuid-ou-ulid-pour-un-index-de-base-de-donnees) par exemple) et passe-le dans le command.
...
Le pattern CQRS/Event-driven avec Messenger est un excellent point de départ pour structurer des applications Symfony maintenables. Si tu veux approfondir le sujet, la [documentation officielle de Symfony Messenger](https://symfony.com/doc/current/messenger.html) est complète et bien structurée, et le [dépôt GitHub du composant](https://github.com/symfony/messenger) permet de lire le code source pour comprendre les internals.

L'architecture hexagonale avec Messenger n'est pas une mode. C'est une approche éprouvée qui rend ton code testable, maintenable et évolutif. Notre accompagnement en [architecture hexagonale Symfony](/architecture-hexagonale-symfony) aide les équipes à mettre en place ces patterns sur des projets existants. Le framework devient un détail d'implémentation. Le domaine reste pur. Et tes collègues te remercieront quand ils ouvriront le projet dans six mois. Si tu pars d'un projet existant et que tu veux adopter cette architecture progressivement, le [retour de mission sur la migration vers l'architecture hexagonale](/article/migration-symfony-architecture-hexagonale-retour-mission) offre une démarche pragmatique issue du terrain.
...
- [Quelle architecture choisir : micro-service ou monolithe modulaire ?](/article/quelle-architecture-de-projet-choisir-entre-micro-service-ou-monolithe-modulaire), choisir l'architecture qui accueillera vos bus
- [Tout savoir sur la mise en cache](/article/tout-savoir-sur-la-mise-en-cache-tips), coupler le cache et Messenger pour l'invalidation asynchrone
- [RAG avec Symfony AI et Doctrine](/article/rag-symfony-ai-doctrine-indexer-base-metier), utiliser Messenger pour l'indexation asynchrone des embeddings
- [Prendre int, UUID ou ULID pour un index de base de données](/article/prendre-int-uuid-ou-ulid-pour-un-index-de-base-de-donnees), choisir les identifiants de vos commands et events
